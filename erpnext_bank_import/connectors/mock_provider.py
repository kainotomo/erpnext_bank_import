...
class MockProvider(BankConnector):
	...
	* ``total_pages`` (default ``3``) — how many pages of transactions
	...
	def normalize_transaction(self, raw: dict[str, Any]) -> NormalizedTransaction:
		...
		if missing:
			raise NormalizationError(f"Missing required fields: {sorted(missing)}")

		# Validate types at a basic level.
		if not isinstance(raw["amount"], (int, float)):
			raise NormalizationError(f"amount must be numeric, got {type(raw['amount']).__name__}")
		...
		return NormalizedTransaction(
			...
			if raw.get("reference_number") is not None
			else None,
			bank_party_name=str(raw["bank_party_name"]) if raw.get("bank_party_name") is not None else None,
			bank_party_account_number=str(raw["bank_party_account_number"])
			if raw.get("bank_party_account_number") is not None
			else None,
			bank_party_iban=str(raw["bank_party_iban"]) if raw.get("bank_party_iban") is not None else None,
			transaction_type=str(raw["transaction_type"])
			if raw.get("transaction_type") is not None
			else None,
